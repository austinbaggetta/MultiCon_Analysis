...
todays_mazes = pd.read_csv(f'../../../{parent_dir}/{experiment_dir}/maze_yml/CircleTrack_{experiment_dir} - TodaysMazes.csv')
todays_mazes_type2 = pd.read_csv(f'../../../{parent_dir}/{experiment_dir}/maze_yml/CircleTrack_{experiment_dir} - TodaysMazes2.csv')
...
for mouse in mouse_list:
    ...
    for i, session in tqdm(enumerate(todays_mazes.columns[1:]), leave=False): ## start from index 1 since index 0 is mouseID
        if (i < starting_idx) | (todays_mazes.loc[todays_mazes['Mouse'] == mouse, session].to_numpy()[0] == 'DONE'):
            pass
        else:
            print(session)
