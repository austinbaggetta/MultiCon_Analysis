...
minian_path = os.path.abspath(f'../../../{project_dir}/{experiment_dir}/minian_results_linear')
...
for mouse in mouse_list:
    ...
    for idx, session in tqdm(enumerate(natsorted(os.listdir(mpath)))):
        ...
        spath = pjoin(output_path, f'aligned_minian_linear/{mouse}/{session_type}')
        if not os.path.exists(spath):
            os.makedirs(spath)

        file_name = os.listdir(pjoin(output_path, f'lin_behav/{mouse}'))[idx]
        behav = pd.read_feather(pjoin(output_path, f'lin_behav/{mouse}/{file_name}'))
