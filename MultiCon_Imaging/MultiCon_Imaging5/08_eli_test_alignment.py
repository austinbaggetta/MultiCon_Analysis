...
## Eli re-processed a mouse to see if reward over-rep still existed.
## This will align his re-processed Minian to behavior
project_dir = 'MultiCon_Imaging'
experiment_dir = 'MultiCon_Imaging5'
minian_path = os.path.abspath(f'Y:/Elijah/Eli_minian_Austin/results')
...
for mouse in mouse_list:
    ...
    for idx, session in tqdm(enumerate(natsorted(os.listdir(mpath)))):
        ...
        spath = pjoin(output_path, f'eli_test/aligned_minian/{mouse}/{session_type}')
        if not os.path.exists(spath):
            os.makedirs(spath)

        file_name = 'mc51_16.feat' ## session I gave Eli to re-process
        behav = pd.read_feather(pjoin(output_path, f'behav/{mouse}/{file_name}'))
        save_path = pjoin(spath, f'{mouse}_{session_type}_{16}.nc') ## hard code because I know the day
        ## Select session
