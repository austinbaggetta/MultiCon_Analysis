...
cohort_name = 'mp1'
mouse_list = [f'mp0{x}' for x in np.arange(1, 10)] + ['mp10', 'mp11', 'mp12']
str2match = "(mp[0-9]+)" ## Set str2match variable (regex for mouse name)
    ...
    for i, session in tqdm(enumerate(todays_mazes.columns[1:]), leave=False): ## start from index 1 since index 0 is mouseID
        if i < starting_idx:
            ...
            if len(rewards) == 2:
                ...
                data_out[['reward_one', 'reward_two', 'reward_three', 'reward_four']] = int(rewards[0][-1]), int(rewards[1][-1]), int(rewards[2][-1]), int(rewards[3][-1])
            elif len(rewards) == 6:
                data_out[['reward_one', 'reward_two', 'reward_three', 'reward_four', 'reward_five', 'reward_six']] = int(rewards[0][-1]), int(rewards[1][-1]), int(rewards[2][-1]), int(rewards[3][-1]), int(rewards[4][-1]), int(rewards[5][-1])    
            data_out = (
