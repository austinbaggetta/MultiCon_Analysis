...
experiment_dir = 'BehaviorCohort1'
todays_mazes = pd.read_csv(os.path.abspath(f'../../../{parent_dir}/{experiment_dir}/maze_yml/todays_mazes.csv'))
behavior_path = os.path.abspath(f"../../../{parent_dir}/{experiment_dir}/")
...
cohort_name = 'bc1'
mouse_list = [f'ms0{x}' for x in np.arange(1, 10)] + [f'ms{x}' for x in np.arange(10, 41)]
...
for mouse in mouse_list:
    ...
    for i, session in tqdm(enumerate(todays_mazes.columns[1:]), leave=False): ## start from index 1 since index 0 is mouseID
        if todays_mazes[session][todays_mazes['Mouse'] == mouse].tolist()[0] == 'DONE':
            ...
            if type(reward_ports) == pd.DataFrame:
                rewards = [x for x in reward_ports['data']]
            else:
                rewards = reward_ports
            ...
            if not rewards[0] and not rewards[1]:
                data_out[['reward_one', 'reward_two']] = np.nan, np.nan
            elif not rewards[0] or not rewards[1]:
                try:
                    data_out[['reward_one', 'reward_two']] = int(rewards[0][-1]), np.nan
                except:
                    data_out[['reward_one', 'reward_two']] = np.nan, int(rewards[1][-1])
            else:
