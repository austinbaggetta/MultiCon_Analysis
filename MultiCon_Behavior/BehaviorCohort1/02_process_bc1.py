...
# %%
## Set parameters
todays_mazes = pd.read_csv(os.path.abspath('../../../MultiCon_Behavior/BehaviorCohort1/maze_yml/todays_mazes.csv'))
behavior_path = os.path.abspath("../../../MultiCon_Behavior/BehaviorCohort1/")
output_path = os.path.abspath("../../../MultiCon_Behavior/BehaviorCohort1/output/behav")
cohort_name = 'bc1'
# mouse_list = [f'ms0{num}' for num in np.arange(7, 10)]
# mouse_list = [f'ms{num}' for num in np.arange(29, 41)]
mouse_list = [f'ms{num}' for num in np.arange(23, 24)]
downsample = False
if downsample:
...
csv_path = pjoin(behavior_path, "circletrack_data/**/**/**/circle_track.csv")
log_path = pjoin(behavior_path, "circletrack_data/**/**/**/**.log")
...
for mouse in mouse_list:
    ...
    for i, session in tqdm(enumerate(todays_mazes.columns[1:]), leave=False): ## start from index 1 since index 0 is mouseID
        if todays_mazes[session][todays_mazes['Mouse'] == mouse].tolist()[0] == 'DONE':
            ...
            if not reward_one and not reward_two:
                data_out[['reward_one', 'reward_two']] = np.nan, np.nan
            elif not reward_one or not reward_two:
                data_out[['reward_one', 'reward_two']] = int(rewards[0][-1]), np.nan
            else:
