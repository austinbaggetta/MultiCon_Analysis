...
experiment_dir = 'MultiCon_Pilot2'
...
cohort_name = 'mcp2'
mouse_list = ['mcp04', 'mcp06', 'mcp07', 'mcp09', 'mcp11', 'mcp12']
str2match = "(mcp[0-9]+)" ## Set str2match variable (regex for mouse name)
downsample = False
        ...
        for i, session in tqdm(enumerate(todays_mazes.columns[1:]), leave=False): ## start from index 1 since index 0 is mouseID
            if i < starting_idx:
                ...
                if todays_mazes[session][todays_mazes['Mouse'] == mouse].tolist()[0] == 'DONE':
                    ...
                    data_out['reward_one'] = int(rewards[0][-1])
                    data_out = (
