...
for mouse in mouse_list:
    ...
    for i, session in tqdm(enumerate(todays_mazes.columns[1:]), leave=False): ## start from index 1 since index 0 is mouseID
        if i < starting_idx:
            pass
        elif any(todays_mazes[session][todays_mazes['Mouse'] == mouse] == 'DONE'):
            adjust_val += 1
            pass
        else:
            print(session)
            circle_track = pd.read_csv(subset[i - adjust_val])
            ...
            if pd.isna(subset_log[i - adjust_val]):
                data_out['maze'] = 'No behavior log'
            else:
                data_out['maze'] = subset_log[i - adjust_val][-9:-4]
